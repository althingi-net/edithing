...
export const Editor: FC<Props> = (props) => {
    const { slate, originalDocument, xml, readOnly, saveDocument, publishDocument, bill, navigationBlocker, t } = props;
    const hasHighlight = useEditorConfig(state => state.highlightStructure);
    const editor = useMemo(() => createEditor(), []);
    const { handleChange, handleSave, handlePublish } = useEditorNavigationBlock(editor, navigationBlocker, saveDocument, publishDocument);
        ...
        <Slate editor={editor} initialValue={slate} onChange={handleChange}>
            <div style={{ height: 'calc(100vh - 104px)' }}>
                <Row gutter={16} style={{ height: '100%' }}>
                    ...
                    <Col span={12}>
                        { readOnly ? null : <Toolbar saveDocument={handleSave} publishDocument={handlePublish} bill={bill} t={t} navigationBlocker={navigationBlocker} /> }
                        <EditorSidePanel
                            t={t}
                            readOnly={readOnly}
                            xml={xml}
