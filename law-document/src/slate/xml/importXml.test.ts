...
describe('importXml', () => {
    ...
    test('empty list item', () => {
        ...
        const imported = importXml(input);

        expect(imported).toStrictEqual(output);
    });
    ...
    test('nodes without configured title or name in tags should not have an empty text node if they have other content', () => {
        const input = `
            <law law-type="law">
                <art nr="1">
                    <paragraph nr="1">
            ...
            createList(MetaType.ART, {}, [
                createListItem(MetaType.ART, '1', {}, [
                    createList(MetaType.PARAGRAPH, {}, [
                        createListItem(MetaType.PARAGRAPH, '1', { text: 'Sendiráð skulu.' }),
                    ]),
                ]),
            ]),
        ];
    ...
    test('ensure after editor normalization, content stays the same (if not it means there is a bug in the import)', () => {
        const input = `
            <?xml version="1.0" encoding="utf-8"?>
            <law law-type="law" nr="93" year="2022">
